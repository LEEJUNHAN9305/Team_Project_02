...
form_window = uic.loadUiType('./mat_zip.ui')[0]
okt = Okt()
...
class Exam(QWidget, form_window):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.lbl_bg.setPixmap(QtGui.QPixmap('./Image/mat_zip_BG.png'))
        self.lbl_result_bg.setPixmap(QtGui.QPixmap('./Image/predict_BG.png'))
        self.lbl_review.setPixmap(QtGui.QPixmap('./Image/search_txt.png'))
        self.btn_predict.setIcon(QtGui.QIcon('./Image/search_icon.png'))
    ...
    def btn_clicked_slot(self):
        ...
        print(evaluation)
        if evaluation == 2:
            self.pixmap.load('./Image/2stars.png')
            self.score.setPixmap(self.pixmap)
            print('debug2')
            self.lbl_result.setText('별로예요')
            print('debug2')
        elif evaluation == 3:
            self.pixmap.load('./Image/3stars.png')
            self.score.setPixmap(self.pixmap)
            print('debug3')
            self.lbl_result.setText('그저 그래요')
            print('debug3')
        elif evaluation == 4:
            self.pixmap.load('./Image/4stars.png')
            self.score.setPixmap(self.pixmap)
            self.lbl_result.setText('좋아요')
        elif evaluation == 5:
            self.pixmap.load('./Image/5stars.png')
            self.score.setPixmap(self.pixmap)
