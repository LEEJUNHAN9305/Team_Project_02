    ...
    while flag2:  #   3, 32
        ...
        try:
            ...
            driver.find_element_by_xpath(
                f'//*[@id="layer_area_box"]/div[2]/div[2]/div/div/div[2]/ul/li[{cnt}]/a').click()
            time.sleep(0.2)
        df_selection_titles = pd.DataFrame(star_scores, columns=['star_scores'])
        df_selection_titles['reviews'] = reviews
        df_titles = pd.concat([df_titles, df_selection_titles], ignore_index=True)
        df_selection_titles.to_csv('./Crawling_data/siksin_crawling_{}.csv'.format(l, datetime.datetime.now().strftime('%Y%m%d')), index=False)
